...
from Utils import Ploter, Scripts, Constraint
...
for fold_num in range(opt.Kf):
    ...
    for subject in range(1, opt.Ns + 1):
        ...
        labels, predicted_labels = CCA.cca_classify(targets, eeg_test, train_data=eeg_train, template=True)
...
Ploter.plot_save_Result(final_acc_list, model_name='CCA', dataset=save_dataset, UD=opt.UD, ratio=opt.ratio, win_size=str(opt.ws),
                        text=True)
