...
'''
      ...
      Direction:  100   10    400    4    54    20    0.0001
           ...
           BETA:  250    9    160    40   70    20    0.0001
'''
...
for fold_num in range(opt.Kf):
    ...
    for subject in range(1, opt.Ns + 1):
        ...
        # Define Network
        net = EEGNet.EEGNet(opt.Nc, int(opt.factor * opt.Fs * opt.ws), opt.Nf)
        net = net.to(devices)
...
Ploter.plot_save_Result(final_acc_list, model_name='EEGNet', dataset=opt.dataset, UD=opt.UD, ratio=opt.ratio,
