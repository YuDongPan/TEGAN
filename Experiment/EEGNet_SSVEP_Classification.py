...
for fold_num in range(opt.Kf):
    ...
    for subject in range(1, opt.Ns + 1):
        ...
        if opt.dataset == 'Direction':
            ...
            train_dataset = EEGDataset.getSSVEP4Intra(subject, KFold=fold_num, n_splits=opt.Kf, mode="test")
            test_dataset = EEGDataset.getSSVEP4Intra(subject, KFold=fold_num, n_splits=opt.Kf, mode="train")
            
            '''Inter-subject Classification Experiment'''
            # train_dataset = EEGDataset.getSSVEP4Inter(subject, mode="train")
            # test_dataset = EEGDataset.getSSVEP4Inter(subject, mode="test")
            ...
            '''Inter-subject Classification Experiment'''
            # train_dataset = EEGDataset.getSSVEP12Inter(subject, mode="train")
            # test_dataset = EEGDataset.getSSVEP12Inter(subject, mode="test")
            
        eeg_train, label_train = train_dataset[:]
        eeg_test, label_test = test_dataset[:]
        eeg_train = eeg_train[:, :, :, :math.ceil(opt.Fs * opt.ws)]
        eeg_test = eeg_test[:, :, :, :math.ceil(opt.Fs * opt.ws)]

        # -------------------------------Augmentation with TEGAN -----------------------------------------------------
#         eeg_generator = TEGAN.Generator(opt.Nc, math.ceil(opt.Fs * opt.ws), opt.Nf, opt.ws, factor=opt.factor)
...
#         n_times = 1
#         eeg_gen_train = torch.zeros((eeg_train.shape[0] * n_times, 1, eeg_train.shape[2], eeg_train.shape[3] * opt.factor))
...
#         eeg_test = eeg_generator(eeg_test.float())[0].detach()
        ...
        # Define Network
        net = EEGNet.EEGNet(opt.Nc, math.ceil(opt.Fs * opt.ws) * opt.factor, opt.Nf)
        net = net.to(devices)
...
Ploter.plot_save_Result(final_acc_list, model_name='EEGNet', dataset=save_dataset, UD=opt.UD, ratio=opt.ratio, win_size=str(opt.ws),
                        text=True)