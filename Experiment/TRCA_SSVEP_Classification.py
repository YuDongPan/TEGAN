...
parser.add_argument('--Kf', type=int, default=2, help="k-fold cross validation")
...
parser.add_argument('--ratio', type=int, default=2, help="-1(Training-free),0(N-1vs1),1(R1),2(5v5),3(2v8)")
...
for fold_num in range(opt.Kf):
    ...
    for subject in range(1, opt.Ns + 1):
        ...
        '''Target Generator'''
        # eeg_generator.load_state_dict(torch.load(f'../Pretrain/{opt.dataset}/{opt.ws}S-{opt.ws * opt.factor}S/'
        #                                        f'Target/ratio{opt.ratio}/TEGAN_Gt_S{subject}.pth'))
        # eeg_generator.eval()
        # z = eeg_train.float()
        #
