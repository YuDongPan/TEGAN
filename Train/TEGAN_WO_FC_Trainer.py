...
def train_on_batch(opt, epochs, train_iter, G, D, device, subject=1, source=0, fold_num=0, lr_jitter=False):
    ...
    for epoch in range(epochs):
        ...
        for i, (eeg, labels) in enumerate(train_iter):
            ...
            ce_loss_gen = 0.5 * ce_loss(pred_label, labels)
    ...
    if source == 0:
        torch.save(G.state_dict(),
                   f'../Pretrain/{opt["Ds"]}/{opt["ws"]}S-{opt["ws"] * opt["F"]}S/TEGAN_FC_Gs_S{subject}.pth')
        torch.save(D.state_dict(),
                   f'../Pretrain/{opt["Ds"]}/{opt["ws"]}S-{opt["ws"] * opt["F"]}S/TEGAN_FC_Ds_S{subject}.pth')

    else:
        torch.save(G.state_dict(), f'../Pretrain/{opt["Ds"]}/{opt["ws"]}S-{opt["ws"] * opt["F"]}S/'
                                   f'TEGAN_FC_Gt_S{subject}_R{opt["ratio"]}F{fold_num}.pth')
