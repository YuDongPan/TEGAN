...
def train_on_batch(opt, train_iter, G, D, device, subject=1, source=0, fold_num=0, lr_jitter=False):
    ...
    d_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(d_optimizer, T_max=opt["epochs"] * len(train_iter), eta_min=5e-6)
    g_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(g_optimizer, T_max=opt["epochs"] * len(train_iter), eta_min=5e-6)
    ...
    for epoch in range(opt["epochs"]):
        ...
        for i, (eeg, labels) in enumerate(train_iter):
            ...
            ce_loss_gen = ce_loss1(pred_label, labels) + ce_loss2(fake_label, labels)
    ...
    if source == 0:
        torch.save(G.state_dict(), f'../Pretrain/{opt["Ds"]}/{opt["ws"]}S-{opt["ws"]*opt["F"]}S/TEGAN_Gs_S{subject}.pth')
        torch.save(D.state_dict(), f'../Pretrain/{opt["Ds"]}/{opt["ws"]}S-{opt["ws"]*opt["F"]}S/TEGAN_Ds_S{subject}.pth')

    else:
        torch.save(G.state_dict(), f'../Pretrain/{opt["Ds"]}/{opt["ws"]}S-{opt["ws"]*opt["F"]}S/'
                                   f'TEGAN_Gt_S{subject}_R{opt["ratio"]}F{fold_num}.pth')
