...
class Discriminator(nn.Module):
    ...
    def forward(self, x):
        ...
        aux_out = self.aux(d_out)
        return real_fake, aux_out
...
class UpBlock(nn.Module):
    def __init__(self, in_channels, out_channels, final_channels, kernel, stride, skip_connection=True,
                 spatial=False):
...
class Generator(nn.Module):
    def __init__(self, Nc, Nt, Nf, ws, factor):
        ...
        self.up_k[1] = self.DF1 - (self.UF1 - 1) * self.S[1]
        self.UF2 = (self.UF1 - 1) * self.S[1] + self.up_k[1]           # 45        121
        self.up_k[2] = self.Nt1 - (self.UF2 - 1) * self.S[0]
    ...
    def forward(self, X):
        ...
        U1 = self.up_stage1(r_out)
        U2 = self.up_stage2(torch.cat([D3, U1], dim=1))
        U3 = self.up_stage3(torch.cat([D2, U2], dim=1))
        U4 = self.up_stage4(torch.cat([D1, U3], dim=1))
        out = self.extent_block(torch.cat([skip_X, U4], dim=1))
